...
void displayQueue(Queue *q) {
    if (q->front > q->rear) {
        printf("Queue is empty. No pending operations.\n");
        return;
    }
...
void displayStack(Stack *s) {
    if (s->top < 0) {
        printf("Stack is empty. No operation history.\n");
        return;
    }
    
    printf("\n==================== OPERATION HISTORY (UNDO) ====================\n");
    for (int i = s->top; i >= 0; i--) {
        printf("ID: %d | Previous Status: %s | Action: %s\n", 
            s->items[i].ticketID, 
            s->items[i].previousStatus, 
            s->items[i].action);
    }
    printf("==================================================================\n");
}

// BUBBLE SORT (Sort Tickets by Ticket ID in Ascending Order)
void bubbleSortTickets(Ticket **head) {
...
//Display Tickets Sorted by Bubble Sort
void displaySortedHistory() {
    if (!ticketHead) {
        printf("\nNo tickets found.\n");
        return;
...
//ISSUE TICKET
void issueTicket() {
    int fIdx, tIdx, type, trip, hasID, i;
    ...
    for (i = 0; from[i]; i++) from[i] = toupper(from[i]);
    for (i = 0; to[i]; i++) to[i] = toupper(to[i]);

    //Validation (Linear Search)
    ...
    if (fIdx < tIdx)
        strcpy(newTicket->tripTime, stations[fIdx].southBound[trip - 1]);
    else
        strcpy(newTicket->tripTime, stations[fIdx].northBound[trip - 1]);
    ...
    while (1) {
        ...
        if (!ticketHead) {
           ...
           //Recent Ticket
            ...
            printf("============================================================\n");
            printf(" TICKET ID: %-12d DATE: %s\n", last->ticketID, last->issueDate);
            printf(" PASSENGER: %s\n", last->passengerName);
        ...
        printf("3. View Operation Queue\n");
        printf("4. View Operation History (Undo Stack)\n");
        printf("5. View Sorted History (Bubble Sort)\n");
        printf("6. Reset All Records\n");
        ...
        if (choice == 1) {
           ...
           //queue display
            ...
            //stack display
            displayStack(&operationHistory);
        }
        else if (choice == 5) {
           //Bubble Sort (Sort Tickets by ID)
            bubbleSortTickets(&ticketHead);
            displaySortedHistory();
        }
        else if (choice == 6) {
            if (ticketHead == NULL) {
...
void cancelTicket() {
    int id;
    Ticket *temp = ticketHead, *prev = NULL;
...
void showFareList() {
    ...
    for (i = 0; i < 12; i++) {
        printf("%-12s : PHP %.2f\n", stations[i].stationName, stations[i].baseFare);
    }
}
